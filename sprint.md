...
- [x] **仿真引擎 + persona 库**:
      ...
      **区分度初验通过** — strong overall=93.0 (coverage 0.9/0.9) vs weak 63.3
      ...
      冒烟顺手抓到两个待查问题 (转 task 2/4 复核): ① strong 场 coverage 达标
      仍 needs_human_review=True, 触发规则待查; ② 强弱两场 comp:tech 维度分
      **同为 95.0** — Evaluator 维度分疑似过誉/饱和 (weak persona 答风模糊却拿
      95), overall 的区分主要靠 comp:comm 拉开, 正是 task 4 报告忠实性 judge
      要核的现象。运维注意: sim 与 uvicorn 并发访问 Milvus Lite 会触发
