...
# Sprint 6.5 F7 (judge 审计发现): 旧 prompt 无忠实性约束, summary 系统性美化 ——
# 复制粘贴场 (53 分) 被写"技术深度表现优异", 跑题场被写"量化结果有效体现"。
# 硬化三条: 只陈述记录事实 / 评价与回答质量一致 / 亮点须可溯源。
# 改本 prompt = 重跑 sim.judge 报告忠实性审计 (judge 已过金标校准)。
_FAITHFUL_RULES = (
    "硬约束:\n"
    "1. 只陈述面试记录中真实出现过的内容, 禁止引入记录外的成果、数字、项目。\n"
    "2. 评价必须与回答的实际质量一致: 回答缺乏个人细节、跑题、敷衍时必须如实"
    "指出, 禁止礼貌性溢美 (不许给低质量回答写\"表现优异/理解深入\")。\n"
    "3. 每个亮点都必须能对应到候选人的某条具体回答, 说不出出处的不写。\n"
)
